# 面向越南留学生的铁道专业汉越术语翻译学习软件源代码

## 一、源代码属性结构说明

1. 软件名称：面向越南留学生的铁道专业汉越术语翻译学习软件 V1.0

2. 项目技术栈：H5、Vue3、Vite、Node.js、Express、MySQL、JavaScript、HTML、CSS。

3. 源码根目录：当前项目根目录，源码稿仅列出相对路径，不包含真实服务器路径。

4. 一级目录划分：server 为 Node.js 后台源码，是本源码稿重点；client 为 H5 前端源码；根目录配置用于统一工作区管理。

5. 二级目录划分：server/src 存放后台配置、数据库、鉴权中间件、路由和工具；server/scripts 存放测试数据和接口测试脚本；client/src 存放页面、接口、国际化和样式。

6. 三级目录划分：server/src/routes、server/src/db、server/src/middleware、server/src/utils 是后台核心目录；client/src/api、client/src/locales、client/src/assets 是前端支撑目录。

7. 选入范围说明：本文优先列出后台服务端源码，包括数据库初始化、鉴权、分页、认证、术语、练习、分析、系统管理和测试脚本；随后列出前端 H5 页面与接口代码；排除第三方依赖、构建产物、截图、PDF、锁文件和二进制资源。

8. 脱敏说明：源码稿中的数据库地址、密码、密钥、邮箱和网络地址已替换为 *** 或等价占位符，脱敏不影响代码结构和模块关系理解。
...
1. 文件路径：server/package.json

2. 文件职责：定义 Express、MySQL、JWT、测试脚本和造数脚本依赖。
...
1. 文件路径：server/.env.example

2. 文件职责：展示服务端口、数据库和鉴权配置项，源码稿中已脱敏。
...
### 3.3 server/src/config/env.js

1. 文件路径：server/src/config/env.js

2. 文件职责：读取服务端口、数据库和鉴权相关环境变量。
...
### 3.4 server/src/db/pool.js

1. 文件路径：server/src/db/pool.js

2. 文件职责：创建数据库、初始化连接池并提供查询函数。
...
### 3.5 server/src/db/init.js

1. 文件路径：server/src/db/init.js

2. 文件职责：创建核心业务表并写入基础角色、账号、分类和术语数据。
...
### 3.6 server/src/db/schema.sql

1. 文件路径：server/src/db/schema.sql

2. 文件职责：以 SQL 形式列出后台 MySQL 核心表结构，便于部署和审阅数据层设计。
...
### 3.7 server/src/middleware/auth.js

1. 文件路径：server/src/middleware/auth.js

2. 文件职责：解析访问凭证、加载用户角色并限制角色权限。
...
### 3.8 server/src/utils/asyncHandler.js

1. 文件路径：server/src/utils/asyncHandler.js

2. 文件职责：统一包装异步路由并传递异常。
...
### 3.9 server/src/utils/pagination.js

1. 文件路径：server/src/utils/pagination.js

2. 文件职责：解析分页参数并生成统一分页返回结构。
...
### 3.10 server/src/routes/auth.js

1. 文件路径：server/src/routes/auth.js

2. 文件职责：实现登录、个人信息、修改密码和退出接口。
...
### 3.11 server/src/routes/terms.js

1. 文件路径：server/src/routes/terms.js

2. 文件职责：实现分类、术语查询、新增、编辑、删除和收藏接口。
...
### 3.12 server/src/routes/practice.js

1. 文件路径：server/src/routes/practice.js

2. 文件职责：实现练习出题、提交判分、练习记录和错题查询接口。
...
### 3.13 server/src/routes/analysis.js

1. 文件路径：server/src/routes/analysis.js

2. 文件职责：实现学生端和管理端学习数据分析接口。
...
### 3.14 server/src/routes/system.js

1. 文件路径：server/src/routes/system.js

2. 文件职责：实现用户、角色、登录日志和操作日志接口。
...
### 3.15 server/src/index.js

1. 文件路径：server/src/index.js

2. 文件职责：组装中间件、业务路由、健康检查和数据库初始化。
...
### 3.16 server/scripts/seed-test-data.js

1. 文件路径：server/scripts/seed-test-data.js

2. 文件职责：生成不少于一百条的业务测试数据。
...
### 3.17 server/scripts/smoke-test.js

1. 文件路径：server/scripts/smoke-test.js

2. 文件职责：验证认证、分页、术语、练习、分析和系统管理接口。
...
1. 文件路径：client/package.json

2. 文件职责：定义 H5 前端 Vue3、Vite 构建和开发脚本。
...
1. 文件路径：client/vite.config.js

2. 文件职责：配置 Vite 插件、端口和接口代理。
...
### 4.3 client/index.html

1. 文件路径：client/index.html

2. 文件职责：定义 H5 应用挂载节点和页面标题。
...
### 4.4 client/src/main.js

1. 文件路径：client/src/main.js

2. 文件职责：创建 Vue 应用并挂载根组件。
...
### 4.5 client/src/api/http.js

1. 文件路径：client/src/api/http.js

2. 文件职责：封装请求、访问凭证保存和清理逻辑。
...
### 4.6 client/src/locales/messages.js

1. 文件路径：client/src/locales/messages.js

2. 文件职责：维护中文、英文、越南语界面文案。
...
### 4.7 client/src/App.vue

1. 文件路径：client/src/App.vue

2. 文件职责：实现登录、导航、术语管理、练习、分析和系统管理页面逻辑。
...
### 4.8 client/src/assets/styles.css

1. 文件路径：client/src/assets/styles.css

2. 文件职责：实现 H5 竖屏和后台横屏响应式布局。
